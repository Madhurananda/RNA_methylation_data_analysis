...
def get_values_tools(tool_name, result_file, ref_content, chr_col_name, pos_col_name, kmer_col_name, prob_col_name, pos_diff_val, n_mer, if_pVal):
    ...
    print('For m6A: only A1 and A2 are considered. These chromosomes are overlooked: ', [x for x in selected_chr_temp if x not in selected_chr])
...
def get_values_Tombo(out_file, ref_conten, tool_name):
...
def get_values_ourtool(result_file, ref_content, tool_name):
    ...
    for selected_chr_FINAL in natsorted(selected_chr):
        ...
        prob_list_0 = []
        prob_list_1 = []
        for a_part in tqdm(result_file.split('\n')):
...
def get_final_array(tool_name, wt_file, ko_file, ref_content, chr_col_name, pos_col_name, kmer_col_name, prob_col_name, pos_diff_val, n_mer, if_pVal):
    if tool_name == 'Tombo':
        neg_array = get_values_Tombo(wt_file[0], ref_content, tool_name)
        pos_array = get_values_Tombo(ko_file[0], ref_content, tool_name)
    elif tool_name == 'OUR':
        log_file_content = read_file(wt_file)
        # ref_gene_content = read_file(ko_file)
        final_array_df = get_values_ourtool(log_file_content, ref_content, tool_name)
    elif tool_name == 'DRUMMER':
    ...
    ## Save the csv file .... 
    save_CSV_file = '/home/madhu/work/comp_tools/paper_6/download_tools/ROC_outputs/roc_info_'+tool_name+'_'+data_token+'.csv'
...
if __name__=='__main__' and '__file__' in globals():
    ...
    # DRUMMER_wt = [file for file in glob.glob(drummer_dir+"/IVT_m6A_0/IVT_m6A_0__isoform/IVT_normalA_fast5_0-IVT_normalA_fast5_1/complete_analysis/*.txt") if 'A1.' in file or 'A2.' in file]
    # DRUMMER_wt = [file for file in glob.glob(drummer_dir+"/IVT_m6A_0/IVT_m6A_0__isoform/IVT_normalA_fast5_0-IVT_normalA_fast5_1/complete_analysis/*.txt") if j in file for j in selected_chr]

    # DRUMMER_wt = []
    # for file in glob.glob(drummer_dir+"/IVT_m6A_0/IVT_m6A_0__isoform/IVT_normalA_fast5_0-IVT_normalA_fast5_1/complete_analysis/*.txt"):
    #     for a_chr in selected_chr:
    #         if a_chr+'.' in file:
    #             DRUMMER_wt.append(file)
    ...
    tombo_wt = glob.glob( tombo_dir+'/'+data_token+'/IVT_m6Afinal_text__.fraction_modified_reads.m6A.plus.wig' )
    tombo_ko = glob.glob( tombo_dir+'/'+data_token+'/IVT_m6Afinal_text__.fraction_modified_reads.m6A.minus.wig' )

    log_file_name = '/home/madhu/Logs/predict__train_p3_BOTH_pred_p5_m6A_A_BilstmMean.layers3.hs512.F.lr1000.b256.p1000.GPU.ep1.240025.pt__A.log'    
    # log_file_name = '/home/madhu/Logs/predict__train_p3_BOTH_pred_p5_m6A_A_BilstmMean.layers3.hs1024.F.lr1500.b512.p1000.GPU.ep1.240025.pt__A.log'
    ...
    ### DRUMMER
    df_ROC_DRUMMER = get_final_array('DRUMMER', DRUMMER_wt, DRUMMER_ko, ref_content, 'chr_ctrl', 'pos_ctrl', 'ref_ctrl', 'padj', 0, 0, 1)
    ...
    ### OUR DNN TOOL
    df_ROC_OUR = get_final_array('OUR', log_file_name, 0, ref_content, 0,0,0,0,0,0,0)


    # print(df_ROC_xPore['xPore_probs__1'])
    ...
    # # df_ROC = df_ROC_DRUMMER.join(df_ROC_xPore['xPore_probs__0']).join(df_ROC_xPore['xPore_probs__1'])

    print('df_ROC ::::::    ')
    print(df_ROC)

    save_CSV_file = '/home/madhu/work/comp_tools/paper_6/download_tools/ROC_outputs/roc_info_'+data_token+'.csv'
    ...
    # values is a list containing: [y_true, y_score, tool]
    # values = [y_true_DRUMMER, y_score_DRUMMER, 'DRUMMER'] 
