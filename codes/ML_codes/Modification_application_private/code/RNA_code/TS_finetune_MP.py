...
def finetuneTrainer(para_dict, datafolders, is_ft, ref_seq_file, not_use_chr=[], index_file='tfrecord.index', input_batch_size=32, cuda_devices: int=1, cpu_devices: int=20, adam_learning_rate: float =1e-4, ft_learning_rate=1e-6, adam_betas=(0.9,0.999), adam_weight_decay: float=0.01, random_seed = 1,  train_epoch=50, warmup_seq=10000, saved_model=None, downsampling_rate=None):
    ...
    filter_files = []
    if 'filter_file' in para_dict:
        filter_files = pickle.load(open(para_dict["filter_file"], "rb"))
    ...
    while train_epoch_i < train_epoch:
        ...
        fs_dataset_0 = TS_ftdataset_reader.TS_Dataset(datafolders[0], index_file, m_seed=train_epoch_i*random_seed, use_percentage=downsampling_rate, filter_files=filter_files)
        if train_epoch_i==1:
