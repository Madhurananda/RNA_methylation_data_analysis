...
def generate_train_feat(signal_full_info2, p_batch_size, used_cpu_devices):
    ...
    for _i_seq in range(len(signal_full_length)):
        ...
        for _this_cp in range(random_center_pos, signal_full_length[_i_seq].size(1), TS_global.input_len):
            ...
            mask_label.append( copy.deepcopy(signal_full_info2[3][_i_seq][0 , t_mask_pos+_this_cp - (TS_global.input_len//2) ] ))
            ##if _this_cp<=100:
...
def preTrainer(para_dict, datafolder, index_file='tfrecord.index', input_batch_size: int=1024, cuda_devices: int=1, cpu_devices: int=5, adam_learning_rate: float =1e-5, adam_betas=(0.9,0.999), adam_weight_decay: float=0.01, random_seed: int=1,  train_epoch: int=60, warmup_seq: int=1000000, gpu_saved_model=None):
    ...
    # 
    mask_signal_model = TS_biLSTM.SignalPred( ispretrain = True, pretrainmean=False, pretrainbp=True, 
    ...
    while (train_epoch_i < train_epoch): # or (train_epoch_i>=train_epoch and  new_lr>1e-8) :
        ...
        fs_dataset = TS_dataset_reader_bp.TS_Dataset(datafolder, index_file, m_seed=train_epoch_i*random_seed)
        if train_epoch_i==1:
        ...
        for batch_fidx, batch_data in enumerate( fs_dataloader ):
            ...
            for batch_ndx, train_batch in enumerate(train_loader):
                ...
                #print( pred_mask.shape, train_batch.maskLab.to(device).unsqueeze(1).shape)
                #loss = loss_func(pred_mask, train_batch.maskLab.to(device).unsqueeze(1)) 
                loss = loss_func(pred_mask, train_batch.maskLab.to(device) )
                ...
                sum_los += loss.item()
                acc_val += (torch.max(pred_mask, 1)[1]==train_batch.maskLab.to(device)).sum().item()/float( pred_mask.size(0))
...
if __name__=='__main__':
    ...
    para_dict['save_file_pref'] = 'Bilst_bp..layers'+sys.argv[2]+'.hs'+sys.argv[3]+'.lr'+sys.argv[5]+'.b'+sys.argv[6]+'.'
    ...
    para_dict['cpu_devices'] = 15; #20; #25;
    ...
    try:
        ...
        preTrainer(  para_dict = para_dict, datafolder=basefolder+'/tfrecordbp.5M', index_file="tfrecord.index", adam_learning_rate=int(sys.argv[5])/1e7, input_batch_size=int(sys.argv[6]), random_seed=3);
