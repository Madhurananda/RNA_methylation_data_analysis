...
def get_values_xPore(result_file, ref_content, col_name):
    ...
    selected_chr = xPore_df['id'].unique()
    # print(selected_chr)

    for selected_chr_FINAL in selected_chr:
        print()
        if selected_chr_FINAL == 'A1' or selected_chr_FINAL == 'A2':
            ...
            for a_chr in chr_list:
                ...
                for i in range(len(xPore_df__selected)):
                    if xPore_df__selected._get_value(i, 'kmer')[2] == a_chr:
                        ind_l.append(i)
                xPore_df__chr = xPore_df__selected.loc[ind_l, :]

                # print(xPore_df__selected.loc[ind_l, :])
                # xPore_df__chr = xPore_df[xPore_df['kmer'] == '**'+a_chr+'**']

                print('Chr: ', a_chr, ' appears ', len(xPore_df__chr), ' times in the xPore output 5-MERS.')
                ...
                if len(pos_DIFF) > 0:
                    print('The positions where ', a_chr, ' appears in the reference sequence, but not in the xPore output: ', [(x-1) for x in pos_DIFF] )
            ...
            print('For m6A: only A1 and A2 are considered. Given: ', selected_chr_FINAL)
            final_array = 0
...
if __name__=='__main__' and '__file__' in globals():
    ...
    # print('df_ROC_DRUMMER: ', df_ROC_DRUMMER)
    # print(len(df_ROC_DRUMMER))
    ...
    # print('df_ROC_xPore: ', df_ROC_xPore)
    # print(len(df_ROC_xPore))
