...
def calc_AUC__draw_ROC(values, saveFig_title): 
    ...
    for i in range(len(values)): 
        ...
        plt.plot(fpr, tpr, list_c[i], linewidth=2, label='{}: (AUC = {:.4f})'.format(tool, AUC_score))
                ...
                if a_pos in output_pos_list:
                    for i in range(len(df__ncld)):
                        if a_pos == int(df__ncld[pos_col_name].values[i]) :
                            ...
                            if if_pVal == 0:
                                # if df__ncld[prob_col_name].values[i] < 0.2:
                                #     p_val = 1
                                # else:
                                #     p_val = 0
                                # p_val = 0
                                ...
                                # p_val = df__ncld[prob_col_name].values[i]
                                pVal_list.append(  pVal_prob(df__ncld[prob_col_name].values[i]) ) 
                            # pVal_list.append(  pVal_prob(p_val) ) 
                else:
                    pVal_list.insert(a_pos-1, pVal_prob(1))
                    # print( a_pos-1, pVal_prob(0.75) )
...
if __name__=='__main__' and '__file__' in globals():
    ...
    # DRUMMER_wt = [file for file in glob.glob(drummer_dir+"/IVT_m6A_0/IVT_m6A_0__isoform/IVT_normalA_fast5_0-IVT_normalA_fast5_1/complete_analysis/*.txt")]
    # DRUMMER_ko = [file for file in glob.glob(drummer_dir+"/IVT_m6A_1/IVT_m6A_1__isoform/IVT_normalA_fast5_1-IVT_m6A_fast5/complete_analysis/*.txt")]
    ## There are more .txt files than the selecteded chr. So, I need to select them first. 
    ...
    ### DRUMMER
    # df_ROC_DRUMMER = get_final_array('DRUMMER', DRUMMER_wt, DRUMMER_ko, ref_content, 'chr_ctrl', 'pos_ctrl', 'ref_ctrl', 'padj', 0, 0, 1) #0.986
    # df_ROC_DRUMMER = get_final_array('DRUMMER', DRUMMER_wt, DRUMMER_ko, ref_content, 'chr_ctrl', 'pos_ctrl', 'ref_ctrl', 'p_val', 0, 0, 1)  #0.989
    # df_ROC_DRUMMER = get_final_array('DRUMMER', DRUMMER_wt, DRUMMER_ko, ref_content, 'chr_ctrl', 'pos_ctrl', 'ref_ctrl', 'G_test', 0, 0, 0) #0.989

    df_ROC_DRUMMER = get_final_array('DRUMMER', DRUMMER_wt, DRUMMER_ko, ref_content, 'chr_ctrl', 'pos_ctrl', 'ref_ctrl', 'p_values_OR_adj', 0, 0, 1) #0.96
    # df_ROC_DRUMMER = get_final_array('DRUMMER', DRUMMER_wt, DRUMMER_ko, ref_content, 'chr_ctrl', 'pos_ctrl', 'ref_ctrl', 'frac_diff', 0, 0, 1) #0.597
    # df_ROC_DRUMMER = get_final_array('DRUMMER', DRUMMER_wt, DRUMMER_ko, ref_content, 'chr_ctrl', 'pos_ctrl', 'ref_ctrl', 'p_values_OR', 0, 0, 1) #0.9745
    # df_ROC_DRUMMER = get_final_array('DRUMMER', DRUMMER_wt, DRUMMER_ko, ref_content, 'chr_ctrl', 'pos_ctrl', 'ref_ctrl', 'log2_(OR)', 0, 0, 0) #0.81

    # df_ROC_DRUMMER = get_final_array('DRUMMER', DRUMMER_wt, DRUMMER_ko, ref_content, 'chr_ctrl', 'pos_ctrl', 'ref_ctrl', 'ref_fraction_ctrl', 0, 0, 0) #0.5
    # df_ROC_DRUMMER = get_final_array('DRUMMER', DRUMMER_wt, DRUMMER_ko, ref_content, 'chr_ctrl', 'pos_ctrl', 'ref_ctrl', 'ratio_ctrl', 0, 0, 1) #0.5
    # df_ROC_DRUMMER = get_final_array('DRUMMER', DRUMMER_wt, DRUMMER_ko, ref_content, 'chr_ctrl', 'pos_ctrl', 'ref_ctrl', 'odds_ratio', 0, 0, 1) # NaN values ... 
    ...
    # df_ROC = df_ROC_DRUMMER.join(df_ROC_xPore['xPore_probs__0']).join(df_ROC_xPore['xPore_probs__1']).join(df_ROC_CHEUI['CHEUI_probs__0']).join(df_ROC_CHEUI['CHEUI_probs__1']).join(df_ROC_Epinano['Epinano_probs__0']).join(df_ROC_Epinano['Epinano_probs__1']).join(df_ROC_m6Anet['m6Anet_probs__0']).join(df_ROC_m6Anet['m6Anet_probs__1'])
    ...
    # ## Load the CSV file: 
    # save_CSV_file = '/home/madhu/work/comp_tools/paper_6/download_tools/ROC_outputs/roc_info_FINAL.csv'
