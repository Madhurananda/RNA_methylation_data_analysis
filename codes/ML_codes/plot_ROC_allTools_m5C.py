...
if __name__=='__main__' and '__file__' in globals():
    ...
    '''
    This one is about m5C data in paper 5 
    '''
    data_token = 'IVT_m5C'
    ...
    DRUMMER_wt = get_DRUMMER_files( drummer_dir+"//IVT_m5C_0/IVT_m5C_0__isoform/IVT_normalC_fast5_0-IVT_normalC_fast5_1/complete_analysis/*.txt", CHR_SELECTED )
    DRUMMER_ko = get_DRUMMER_files( drummer_dir+"/IVT_m5C_1/IVT_m5C_1__isoform/IVT_normalC_fast5_0-IVT_m5C_fast5/complete_analysis/*.txt", CHR_SELECTED )
    ...
    our_log_file = '/home/madhu/Logs/predict__train_p5_m5C_pred_C1_C_BilstmMean.layers3.hs512.F.lr1500.b512.p1000.GPU.ep1.360121.pt__C.log'
    # our_log_file = '/home/madhu/Logs/predict__train_p5_m5C_pred_C1_C_BilstmMean.layers3.hs512.F.lr1000.b256.p1000.GPU.ep1.260042.pt__C.log'
    ...
    # cheui_wt = glob.glob( cheui_dir+'/IVT_m5C_OLD/WT/WT_site_level_pred.txt' )
    # cheui_ko = glob.glob( cheui_dir+'/IVT_m5C_OLD/KO/KO_site_level_pred.txt' )
    ...
    # df_ROC = df_ROC_DRUMMER.join(df_ROC_CHEUI['CHEUI_probs__0']).join(df_ROC_CHEUI['CHEUI_probs__1']).join(df_ROC_m6Anet['m6Anet_probs__0']).join(df_ROC_m6Anet['m6Anet_probs__1']).join(df_ROC_OUR['OUR_probs__0']).join(df_ROC_OUR['OUR_probs__1'])

    df_ROC = df_ROC_DRUMMER.join(df_ROC_CHEUI['CHEUI_probs__0']).join(df_ROC_CHEUI['CHEUI_probs__1']).join(df_ROC_OUR['OUR_probs__0']).join(df_ROC_OUR['OUR_probs__1'])

    # df_ROC = df_ROC_DRUMMER.join(df_ROC_m6Anet['m6Anet_probs__0']).join(df_ROC_m6Anet['m6Anet_probs__1']).join(df_ROC_OUR['OUR_probs__0']).join(df_ROC_OUR['OUR_probs__1'])

    # df_ROC = df_ROC_m6Anet.join(df_ROC_OUR['OUR_probs__0']).join(df_ROC_OUR['OUR_probs__1'])
    ...
    # values = [[y_true_ours, y_score_ours, 'Our_DNN_tool'], [y_true_m6Anet, y_score_m6Anet, 'm6Anet'], [y_true_DRUMMER, y_score_DRUMMER, 'DRUMMER'], [y_true_CHEUI, y_score_CHEUI, 'CHEUI']]
    values = [[y_true_ours, y_score_ours, 'Our_DNN_tool'], [y_true_DRUMMER, y_score_DRUMMER, 'DRUMMER'], [y_true_CHEUI, y_score_CHEUI, 'CHEUI']]
