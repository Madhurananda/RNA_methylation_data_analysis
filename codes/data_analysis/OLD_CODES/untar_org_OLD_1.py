...
#    print('The list of compressed files: ', compressed_files)
   ...
   results = Pool(n_jobs).imap_unordered(unzip, inputs)
   ...
   # for a_com_file in tqdm(compressed_files[0:2]):
   #    print('\n\n')

   #    # a_com_file = '/home/madhu/datasets/download/p5_decoding_epitranscriptional_native_RNA_seq/original_data/IVT_normalG.fast5.gz'
   #    # out_file = output_folder+a_com_file.split('.')[0].split('/')[-1]+"/"+a_com_file.split('.gz')[0].split('/')[-1]
...
#             # inputs = zip(compressed_files, [extract_dir]*len(compressed_files))
