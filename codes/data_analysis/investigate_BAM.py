...
if __name__ == '__main__':
    ...
    for a_dataset in tqdm(dataset_list):
        ...
        if max(list_query_algn_len) > max_c_X_lim:
            if max(list_query_algn_len) > max_c_X_lim+500:
                list_BIN_range = [0, LEN_THR] + [500, 1000, 1500, max_c_X_lim] + [max(list_query_algn_len)]
            else: 
                list_BIN_range = [0, LEN_THR] + [500, 1000, 1500, max_c_X_lim] + [max_c_X_lim+500]
        else:
        ...
        x = [LEN_THR, LEN_THR]
        y = [0, int(max(counts)*1.05)]
        plt.plot(x, y, color='red', linestyle='--', linewidth=2.5, label='Length Threshold = '+str(LEN_THR))
        plt.plot([], [], ' ', label='N_reads under '+str(LEN_THR)+' is: '+ str(int(counts[0])))
        plt.plot([], [], ' ', label='N_reads over '+str(LEN_THR)+' is: '+ str(int(sum(counts)-counts[0])))
        # plt.plot([], [], ' ', label='Good READ: '+ str( round(((sum(counts)-counts[0])/sum(counts))*100, 2)) + '%' )
        plt.plot([], [], ' ', label='Good READ: {:.2f}%'.format(((sum(counts)-counts[0])/sum(counts))*100)  )
        ...
        if max(list_MAPQ) > max_c_X_lim:
            if max(list_MAPQ) > max_c_X_lim+20:
                list_BIN_range = [0, MQ_THR] + [40, 60, max_c_X_lim] + [max(list_MAPQ)]
            else:
                list_BIN_range = [0, MQ_THR] + [40, 60, max_c_X_lim] + max_c_X_lim+20
        else:
        ...
        x = [MQ_THR, MQ_THR]
        y = [0, int(max(counts)*1.05)]
        plt.plot(x, y, color='red', linestyle='--', linewidth=2.5, label='Mapping Quality Threshold = '+str(MQ_THR))
        plt.plot([], [], ' ', label='N_reads under '+str(MQ_THR)+' is: '+ str(int(counts[0])))
        plt.plot([], [], ' ', label='N_reads over '+str(MQ_THR)+' is: '+ str(int(sum(counts)-counts[0])))
        plt.plot([], [], ' ', label='Good READ: '+ str( round(((sum(counts)-counts[0])/sum(counts))*100, 2)) + '%' )
        plt.legend(fontsize=15, loc='center right')

        fig.suptitle(a_dataset, fontsize=25)
        plt.show()
